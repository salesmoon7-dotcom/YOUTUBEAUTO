...
from runtime_v2.excel.status_contract import TERMINAL_STATUSES, can_transition_excel_status
    ...
    *,
    ...
    try:
        ...
        if status_col is None:
            return False
        row_no = row_index + 2
        current_status = str(sheet.cell(row=row_no, column=status_col).value or "").strip()
        if current_status.lower() in TERMINAL_STATUSES:
            return False
        if not can_transition_excel_status(current_status, next_status):
            return False
        if not _assign_cell(sheet, row_no=row_no, column_no=status_col, value=next_status):
            return False
        if summary_col is not None:
            if not _assign_cell(sheet, row_no=row_no, column_no=summary_col, value=summary):
                return False
        if reason_col is not None:
            if not _assign_cell(sheet, row_no=row_no, column_no=reason_col, value=reason_code):
                return False
        try:
