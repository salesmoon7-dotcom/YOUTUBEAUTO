...
def _run_agent_browser_stage2_adapter_child(args: CliArgs) -> int:
    ...
    if prompt and service == "genspark":
        ...
        actions = [
            ...
            {
                "type": "eval",
                "script": "(() => { const nodes = Array.from(document.querySelectorAll('*')); const isVisible = (el) => !!el && ((el.getClientRects && el.getClientRects().length > 0) || el.offsetParent !== null); const describeBackgroundCandidates = () => Array.from(document.querySelectorAll('button,[role=button],[role=tab],[aria-label]')).map(item => ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim()).filter(text => text.includes('배경') || text.includes('Background')).slice(0, 20); const findByText = (matcher) => nodes.find(node => { const text = ((node.innerText || node.textContent || '') + ' ' + (node.getAttribute?.('aria-label') || '')).trim(); return matcher(text); }); const resolveClickable = (node) => { if (!(node instanceof HTMLElement)) return null; const clickable = node.closest('button,[role=button],[role=tab],a'); if (clickable instanceof HTMLElement && isVisible(clickable)) return clickable; return isVisible(node) ? node : null; }; const rawTarget = findByText((text) => text === 'Product Background' || text.includes('Product Background Product Background')) || findByText((text) => text === '배경' || text.includes('배경 배경')); const target = resolveClickable(rawTarget); if (!(target instanceof HTMLElement)) return JSON.stringify({ok:false,error:'NO_BACKGROUND_SIDEBAR_ITEM', backgroundCandidates: describeBackgroundCandidates()}); target.click(); return JSON.stringify({ok:true, step:'opened_background_sidebar'}); })()",
            },
            {
                "type": "wait",
                "target": "800",
