...
def _run_agent_browser_stage2_adapter_child(args: CliArgs) -> int:
    ...
    if prompt and service == "genspark":
        ...
        actions = [
            ...
            {
                "type": "eval",
                "script": "(async () => { const wait = (ms) => new Promise(resolve => setTimeout(resolve, ms)); const readState = () => { const href = String(location.href || ''); const title = (document.title || '').trim(); const entry = document.querySelector('[aria-label=\"캔버스 진입점\"]'); const entryVisible = !!entry && (((entry.getClientRects && entry.getClientRects().length > 0)) || entry.offsetParent !== null); return { href, title, hasCanvasEntrypoint: !!entry, entryVisible, editorReady: entryVisible || (href.includes('/design/') && title.includes('Canva 편집기') && entryVisible) }; }; let state = readState(); if (state.editorReady) return JSON.stringify({ok:true, step:'canva_editor_ready'}); if (state.href.includes('oauth/authorized')) { await wait(5000); state = readState(); if (state.editorReady) return JSON.stringify({ok:true, step:'canva_editor_ready_after_settle'}); } return JSON.stringify({ok:false,error:'CANVA_EDITOR_NOT_READY', currentUrl: state.href, currentTitle: state.title, hasCanvasEntrypoint: state.hasCanvasEntrypoint, entryVisible: state.entryVisible}); })()",
            },
            {
                "type": "eval",
            ...
            {
                "type": "eval",
                "script": "(async () => { const wait = (ms) => new Promise(resolve => setTimeout(resolve, ms)); const pressEscape = () => { document.dispatchEvent(new KeyboardEvent('keydown', {key:'Escape', code:'Escape', keyCode:27, which:27, bubbles:true})); document.dispatchEvent(new KeyboardEvent('keyup', {key:'Escape', code:'Escape', keyCode:27, which:27, bubbles:true})); }; pressEscape(); await wait(300); pressEscape(); await wait(300); const entry = document.querySelector('[aria-label=\"캔버스 진입점\"]'); if (entry instanceof HTMLElement) { entry.click(); await wait(800); } const buttons = Array.from(document.querySelectorAll('button,[role=button]')); const target = buttons.find(item => { const text = ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim(); return text === '배경 생성' || text.includes('Create background') || text.includes('Background generator'); }); if (!(target instanceof HTMLElement)) return JSON.stringify({ok:true, step:'background_generate_legacy_optional'}); target.click(); return JSON.stringify({ok:true, step:'opened_background_generate_panel_legacy'}); })()",
            },
            {
                "type": "wait",
                "target": "800",
            ...
            {
                "type": "eval",
                "script": "(() => { const isVisible = (el) => !!el && ((el.getClientRects && el.getClientRects().length > 0) || el.offsetParent !== null); const describeBackgroundCandidates = () => Array.from(document.querySelectorAll('button,[role=button],[role=tab],[aria-label]')).map(item => ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim()).filter(text => text.includes('배경') || text.includes('Background')).slice(0, 20); const items = Array.from(document.querySelectorAll('button[role=tab][aria-controls], [role=tab][aria-controls]')); const findTarget = (matcher) => items.find(item => matcher(((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim())); const target = findTarget((text) => text === 'Product Background' || text.includes('Product Background Product Background')) || findTarget((text) => text === '배경' || text.includes('배경 배경')); if (!(target instanceof HTMLElement)) return JSON.stringify({ok:false,error:'NO_BACKGROUND_SIDEBAR_ITEM', backgroundCandidates: describeBackgroundCandidates()}); target.focus(); target.click(); target.dispatchEvent(new KeyboardEvent('keydown', {key:'Enter', code:'Enter', keyCode:13, which:13, bubbles:true})); target.dispatchEvent(new KeyboardEvent('keyup', {key:'Enter', code:'Enter', keyCode:13, which:13, bubbles:true})); target.dispatchEvent(new KeyboardEvent('keydown', {key:' ', code:'Space', keyCode:32, which:32, bubbles:true})); target.dispatchEvent(new KeyboardEvent('keyup', {key:' ', code:'Space', keyCode:32, which:32, bubbles:true})); const panelId = target.getAttribute('aria-controls') || ''; const panel = panelId ? document.getElementById(panelId) : null; const iframe = panel ? panel.querySelector('iframe[title=\"Product Background\"]') : null; const ariaSelected = target.getAttribute('aria-selected') || ''; const panelVisible = !!panel && isVisible(panel); const iframeVisible = !!iframe && isVisible(iframe); if (ariaSelected !== 'true' && !panelVisible && !iframeVisible) return JSON.stringify({ok:false,error:'NO_BACKGROUND_TAB_ACTIVATION', ariaSelected: ariaSelected, panelVisible: panelVisible, iframeVisible: iframeVisible, ariaControls: panelId, backgroundCandidates: describeBackgroundCandidates()}); return JSON.stringify({ok:true, step:'opened_background_sidebar', ariaSelected: ariaSelected, panelVisible: panelVisible, iframeVisible: iframeVisible, ariaControls: panelId}); })()",
            },
            {
                "type": "wait",
                "target": "800",
            ...
            {
                "type": "eval",
                "script": "(async () => { const wait = (ms) => new Promise(resolve => setTimeout(resolve, ms)); const isVisible = (el) => !!el && ((el.getClientRects && el.getClientRects().length > 0) || el.offsetParent !== null); const describeVisibleCandidates = () => Array.from(document.querySelectorAll('button,[role=button],[role=tab],[aria-label]')).map(item => ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim()).filter(Boolean).slice(0, 20); const describeBackgroundCandidates = () => Array.from(document.querySelectorAll('button,[role=button],[role=tab],[aria-label]')).map(item => ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim()).filter(text => text.includes('배경') || text.includes('Background')).slice(0, 20); const openSidebar = () => { const items = Array.from(document.querySelectorAll('button[role=tab][aria-controls], [role=tab][aria-controls]')); const findTarget = (matcher) => items.find(item => matcher(((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim())); const target = findTarget((text) => text === 'Product Background' || text.includes('Product Background Product Background')) || findTarget((text) => text === '배경' || text.includes('배경 배경')); if (!(target instanceof HTMLElement)) return JSON.stringify({ok:false,error:'NO_BACKGROUND_SIDEBAR_ITEM', visibleCandidates: describeVisibleCandidates(), backgroundCandidates: describeBackgroundCandidates()}); target.focus(); target.click(); target.dispatchEvent(new KeyboardEvent('keydown', {key:'Enter', code:'Enter', keyCode:13, which:13, bubbles:true})); target.dispatchEvent(new KeyboardEvent('keyup', {key:'Enter', code:'Enter', keyCode:13, which:13, bubbles:true})); target.dispatchEvent(new KeyboardEvent('keydown', {key:' ', code:'Space', keyCode:32, which:32, bubbles:true})); target.dispatchEvent(new KeyboardEvent('keyup', {key:' ', code:'Space', keyCode:32, which:32, bubbles:true})); return JSON.stringify({ok:true, step:'opened_background_sidebar', visible: isVisible(target), ariaControls: target.getAttribute('aria-controls') || ''}); }; const findExact = () => Array.from(document.querySelectorAll('button,[role=button]')).find(item => ((item.getAttribute('aria-label') || '').trim() === '배경 생성')); const hasGenerateCta = () => Array.from(document.querySelectorAll('button,[role=button]')).some(item => { const text = ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim(); return text === '생성' || text.includes('Generate'); }); const tryRefreshGate = () => { const containers = Array.from(document.querySelectorAll('div,section,[role=dialog],aside')); const stale = containers.find(node => { const text = (node.innerText || node.textContent || '').trim(); return text.includes('새 버전') && text.includes('페이지 새로 고침'); }); if (!(stale instanceof HTMLElement)) return {ok:true, refreshed:false}; const buttons = Array.from(stale.querySelectorAll('button,[role=button],a')); const btn = buttons.find(item => { const text = ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim(); return text.includes('페이지 새로 고침') || text.includes('새로 고침') || text.includes('Refresh'); }); if (!(btn instanceof HTMLElement)) return {ok:false,error:'NO_BACKGROUND_REFRESH_BUTTON'}; btn.click(); return {ok:true, refreshed:true}; }; const sidebar = JSON.parse(openSidebar()); if (!sidebar.ok) return JSON.stringify(sidebar); const deadline = Date.now() + 4000; while (Date.now() < deadline) { const refreshState = tryRefreshGate(); if (!refreshState.ok) return JSON.stringify({ok:false,error:refreshState.error, visibleCandidates: describeVisibleCandidates(), backgroundCandidates: describeBackgroundCandidates()}); const exact = findExact(); if (exact instanceof HTMLElement) { exact.click(); return JSON.stringify({ok:true, step:'opened_background_generate_panel', source:'aria-label'}); } if (hasGenerateCta()) { return JSON.stringify({ok:true, step:'opened_background_generate_panel', source:'generate-cta-only'}); } await wait(refreshState.refreshed ? 800 : 200); } return JSON.stringify({ok:false,error:'NO_BACKGROUND_GENERATE_BUTTON', visibleCandidates: describeVisibleCandidates(), backgroundCandidates: describeBackgroundCandidates()}); })()",
            },
            {
                "type": "eval",
        ...
        actions.extend(
            [
                ...
                {
                    "type": "eval",
                    "script": "(() => { const input = document.querySelector('input[placeholder*=\"페이지\"], input[placeholder*=\"page\"]'); if (!(input instanceof HTMLElement)) return JSON.stringify({ok:true, step:'page_picker_unavailable'}); const targetPage = String(Number(window.__runtime_v2_canva_page_count_after || 0) || Number(window.__runtime_v2_canva_page_count_before || 0) || 2); input.click(); if ('focus' in input) input.focus(); if ('value' in input) { input.value = ''; input.dispatchEvent(new Event('input', {bubbles:true})); input.value = targetPage; input.dispatchEvent(new Event('input', {bubbles:true})); input.dispatchEvent(new Event('change', {bubbles:true})); } input.dispatchEvent(new KeyboardEvent('keydown', {key:'Enter', code:'Enter', keyCode:13, which:13, bubbles:true})); input.dispatchEvent(new KeyboardEvent('keyup', {key:'Enter', code:'Enter', keyCode:13, which:13, bubbles:true})); window.__runtime_v2_canva_created_page = Number(targetPage || '0'); return JSON.stringify({ok:true, step:'typed_current_page', page: targetPage, source:'created-page-explicit'}); })()",
                },
                {
                    "type": "eval",
                ...
                {
                    "type": "eval",
                    "script": "(() => { const createdPage = Number(window.__runtime_v2_canva_created_page || 0); const containers = Array.from(document.querySelectorAll('div,button,[role=button]')).filter(node => node instanceof HTMLElement && (node.offsetWidth > 0 || node.offsetHeight > 0)); const card = createdPage > 0 ? containers.find(node => { const text = (node.textContent || '').trim(); return text.includes(`페이지 ${createdPage}`) || text.includes(`Page ${createdPage}`); }) : null; const deleteFromCard = card instanceof HTMLElement ? Array.from(card.querySelectorAll('button[aria-label=\"페이지 삭제\"], button[aria-label=\"Delete page\"]')).find(node => node instanceof HTMLElement) : null; const buttons = Array.from(document.querySelectorAll('button[aria-label=\"페이지 삭제\"], button[aria-label=\"Delete page\"]')); const fallback = buttons.length >= 2 ? buttons[buttons.length - 1] : null; const btn = (deleteFromCard instanceof HTMLElement ? deleteFromCard : null) || (fallback instanceof HTMLElement ? fallback : null); if (!(btn instanceof HTMLElement)) return JSON.stringify({ok:true, step:'cleanup_skipped_single_page'}); btn.click(); return JSON.stringify({ok:true, step:'cleanup_deleted_created_page', page: createdPage || ''}); })()",
                },
            ]
        )
