...
def _run_agent_browser_stage2_adapter_child(args: CliArgs) -> int:
    ...
    if prompt and service == "genspark":
        ...
        seaart_prompt_selector = "textarea.prompt-input.image-upload-prompt-input:visible"
        seaart_prompt_base_selector = "textarea.prompt-input.image-upload-prompt-input"
        pre_actions = [
            {
                "type": "eval",
        ...
        actions = [
            ...
            {
                "type": "eval",
                "script": "(() => { const picker = document.querySelector('input[placeholder*=\"페이지\"], input[placeholder*=\"page\"]'); if (window.__runtime_v2_canva_duplicated && !(picker instanceof HTMLElement)) return JSON.stringify({ok:true, step:'selected_created_page_optional'}); const nodes = Array.from(document.querySelectorAll('div,button,[role=button],[role=tab]')); const target = nodes.find(node => { const text = (node.innerText || node.textContent || '').trim(); return text === '페이지 2' || text === 'Page 2'; }); if (!(target instanceof HTMLElement)) return JSON.stringify({ok:false,error:'NO_PAGE_2_TARGET'}); target.click(); return JSON.stringify({ok:true, step:'selected_created_page'}); })()",
            },
            {
                "type": "wait",
                "target": "500",
    ...
    if pre_actions:
        ...
        if not stage2_attach_verify_succeeded(pre_result):
            ...
            if (
                ...
                and "textarea.prompt-input.image-upload-prompt-input:visible"
                in pre_error_text
                ...
                _ = write_stage2_attach_evidence(
                    ...
                    ref_images_resolved=ref_images_resolved,
                    extra_details={
                        "selector": "textarea.prompt-input.image-upload-prompt-input:visible",
...
def _build_canva_background_generate_script() -> str:
    return (
        "(async () => { "
        "const wait = (ms) => new Promise(resolve => setTimeout(resolve, ms)); "
        "const isVisible = (el) => !!el && ((el.getClientRects && el.getClientRects().length > 0) || el.offsetParent !== null); "
        "const describeVisibleCandidates = () => Array.from(document.querySelectorAll('button,[role=button],[role=tab],[aria-label]')).map(item => ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim()).filter(Boolean).slice(0, 20); "
        "const describeBackgroundCandidates = () => Array.from(document.querySelectorAll('button,[role=button],[role=tab],[aria-label]')).map(item => ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim()).filter(text => text.includes('배경') || text.includes('Background')).slice(0, 20); "
        "const openSidebar = () => { const items = Array.from(document.querySelectorAll('button[role=tab][aria-controls], [role=tab][aria-controls]')); const findTarget = (matcher) => items.find(item => matcher(((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim())); const target = findTarget((text) => text === 'Product Background' || text.includes('Product Background Product Background')) || findTarget((text) => text === '배경' || text.includes('배경 배경')); if (!(target instanceof HTMLElement)) return JSON.stringify({ok:false,error:'NO_BACKGROUND_SIDEBAR_ITEM', visibleCandidates: describeVisibleCandidates(), backgroundCandidates: describeBackgroundCandidates()}); target.focus(); target.click(); target.dispatchEvent(new KeyboardEvent('keydown', {key:'Enter', code:'Enter', keyCode:13, which:13, bubbles:true})); target.dispatchEvent(new KeyboardEvent('keyup', {key:'Enter', code:'Enter', keyCode:13, which:13, bubbles:true})); target.dispatchEvent(new KeyboardEvent('keydown', {key:' ', code:'Space', keyCode:32, which:32, bubbles:true})); target.dispatchEvent(new KeyboardEvent('keyup', {key:' ', code:'Space', keyCode:32, which:32, bubbles:true})); const panelId = target.getAttribute('aria-controls') || ''; const panel = panelId ? document.getElementById(panelId) : null; const iframe = panel ? panel.querySelector('iframe[title=\"Product Background\"]') : null; const ariaSelected = target.getAttribute('aria-selected') || ''; const panelVisible = !!panel && isVisible(panel); const iframeVisible = !!iframe && isVisible(iframe); return JSON.stringify({ok:true, step:'opened_background_sidebar', ariaSelected: ariaSelected, panelVisible: panelVisible, iframeVisible: iframeVisible, ariaControls: panelId}); }; "
        "const findExact = () => Array.from(document.querySelectorAll('button,[role=button]')).find(item => ((item.getAttribute('aria-label') || '').trim() === '배경 생성')); "
        "const hasGenerateCta = () => Array.from(document.querySelectorAll('button,[role=button]')).some(item => { const text = ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim(); return text === '생성' || text.includes('Generate'); }); "
