...
def _run_agent_browser_stage2_adapter_child(args: CliArgs) -> int:
    ...
    if prompt and service == "genspark":
        ...
        actions = [
            ...
            {
                "type": "eval",
                "script": "(() => { const items = Array.from(document.querySelectorAll('button,[role=button],[role=tab],[aria-label]')); const describeBackgroundCandidates = () => items.map(item => ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim()).filter(text => text.includes('배경') || text.includes('Background')).slice(0, 20); const target = items.find(item => { const text = ((item.innerText || item.textContent || '') + ' ' + (item.getAttribute('aria-label') || '')).trim(); return text === '배경' || text === 'Product Background' || text.includes('배경 배경') || text.includes('Product Background Product Background'); }); if (!(target instanceof HTMLElement)) return JSON.stringify({ok:false,error:'NO_BACKGROUND_SIDEBAR_ITEM', backgroundCandidates: describeBackgroundCandidates()}); target.click(); return JSON.stringify({ok:true, step:'opened_background_sidebar'}); })()",
            },
            {
                "type": "wait",
                "target": "800",
