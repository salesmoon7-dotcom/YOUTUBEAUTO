...
from runtime_v2.stage2.legacy_executor import LEGACY_ROOT, build_geminigen_prompt_file, int_value, load_legacy_result_json, resolve_output_from_result, script_command, stage_output, stage_service_artifact
...
def run_geminigen_job(job: JobContract, artifact_root: Path, registry_file: Path | None = None) -> dict[str, object]:
    if registry_file is not None:
        _ = update_worker_state(registry_file, workload="geminigen", state="busy", run_id=str(job.payload.get("run_id", job.job_id)))
    ...
    command = script_command("geminigen_automation.py") + [
        "--json",
    ...
    if exit_code != 0 or not result_json_path.exists():
        ...
        if parsed_result.get("status") == "failed":
            ...
            if output_path is None:
                ...
                staged_output = stage_output(workspace, output_path, fallback_name="geminigen.mp4")
    ...
    if registry_file is not None:
        _ = update_worker_state(registry_file, workload="geminigen", state="idle", run_id=str(job.payload.get("run_id", job.job_id)))
    return result
