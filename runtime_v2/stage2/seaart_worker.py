...
def run_seaart_job(job: JobContract, artifact_root: Path, registry_file: Path | None = None) -> dict[str, object]:
    if registry_file is not None:
        _ = update_worker_state(registry_file, workload="seaart", state="busy", run_id=str(job.payload.get("run_id", job.job_id)))
    ...
    command = script_command("seaart_automation.py") + [
        "--prompt-file",
        str(prompt_path.resolve()),
        "--row-index",
        str(row_index_from_payload(job.payload)),
        "--model",
        str(job.payload.get("model", "FLUX")),
        "--result-json",
    ...
    if exit_code != 0 or not result_json_path.exists():
        ...
        if parsed_result.get("status") == "failed":
            ...
            if output_path is None:
                ...
                staged_output = stage_output(workspace, output_path, fallback_name="seaart.png")
    ...
    if registry_file is not None:
        _ = update_worker_state(registry_file, workload="seaart", state="idle", run_id=str(job.payload.get("run_id", job.job_id)))
    return result
