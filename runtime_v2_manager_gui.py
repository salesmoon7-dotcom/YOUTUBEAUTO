...
from runtime_v2.control_plane import run_control_loop_once, seed_control_job, seed_local_jobs
from runtime_v2.contracts.job_contract import JobContract
from runtime_v2.debug_log import append_debug_event, debug_log_path, exception_payload, summarize_runtime_result
...
def _archived_contract_count(root: Path) -> int:
    ...
    legacy_job_like = [path for path in root.glob("*.job.*.json") if not path.name.endswith(".reason.json")]
...
class RuntimeV2ManagerGUI:
    ...
    def _build_control_frame(self, parent: ttk.Frame) -> None:
        ...
        combo = ttk.Combobox(top, textvariable=self.selected_workload, state="readonly", width=16)
        combo["values"] = ("qwen3_tts", "rvc", "kenburns")
        combo.pack(side=tk.LEFT, padx=(6, 0))

        row = ttk.Frame(frame)
        row.pack(fill=tk.X, pady=(0, 6))
        tk.Button(row, text="▶ Start", width=10, command=self.start_loop, fg="#1f7a1f").pack(side=tk.LEFT)
        tk.Button(row, text="⏸ Pause", width=10, command=self.toggle_pause, fg="#b36b00").pack(side=tk.LEFT, padx=4)
        tk.Button(row, text="■ Stop", width=10, command=self.stop_loop, fg="#b32020").pack(side=tk.LEFT)
        tk.Button(row, text="↻ Recovery", width=12, command=self.trigger_recovery).pack(side=tk.LEFT, padx=(16, 4))
        tk.Button(row, text="Scan Inbox", width=12, command=self.trigger_scan_inbox).pack(side=tk.LEFT, padx=(4, 4))
        tk.Button(row, text="GPT Spawn", width=12, command=self.trigger_gpt_spawn).pack(side=tk.LEFT)

        seed = ttk.Frame(frame)
        seed.pack(fill=tk.X)
        ...
        ttk.Entry(prompt, textvariable=self.script_text, width=64).pack(side=tk.LEFT, fill=tk.X, expand=True)

        seed_buttons = ttk.Frame(frame)
        seed_buttons.pack(fill=tk.X, pady=(6, 0))
        ttk.Label(seed_buttons, text="큐 삽입:").pack(side=tk.LEFT)
        tk.Button(seed_buttons, text="QWEN3", width=10, command=lambda: self.seed_job("qwen3_tts")).pack(side=tk.LEFT, padx=(6, 4))
        tk.Button(seed_buttons, text="RVC", width=10, command=lambda: self.seed_job("rvc")).pack(side=tk.LEFT, padx=4)
        tk.Button(seed_buttons, text="KenBurns", width=10, command=lambda: self.seed_job("kenburns")).pack(side=tk.LEFT, padx=4)

    def _build_overview_frame(self, parent: ttk.Frame) -> None:
        frame = ttk.LabelFrame(parent, text="Overview", padding=8)
        frame.pack(fill=tk.X, pady=(0, 8))
        ttk.Label(frame, textvariable=self.status_text, font=("Malgun Gothic", 10, "bold")).pack(anchor="w")
        ttk.Label(frame, textvariable=self.warning_text, foreground="#b32020").pack(anchor="w", pady=(4, 0))
        ttk.Label(frame, textvariable=self.last_result_text).pack(anchor="w", pady=(4, 0))

        grid = ttk.Frame(frame)
        grid.pack(fill=tk.X, pady=(6, 0))
        for index, var in enumerate((self.browser_text, self.gpu_text, self.gpt_text, self.queue_text, self.artifact_text)):
            label = ttk.Label(grid, textvariable=var, relief=tk.GROOVE, padding=6)
            _ = label.grid(row=index // 2, column=index % 2, sticky="ew", padx=2, pady=2)
    ...
    def _build_programs_frame(self, parent: ttk.Frame) -> None:
        ...
        frame.pack(fill=tk.X, pady=(0, 8))
        for program in ("GPT", "Genspark", "SeaArt", "Browser", "GeminiGen", "KenBurns", "QWEN3_TTS", "RVC", "Result", "GUI"):
            row = ttk.Frame(frame)
    ...
    def _build_logs_frame(self, parent: ttk.Frame) -> None:
        ...
        self.log_text = tk.Text(frame, height=18, font=("Consolas", 9), bg="#263238", fg="#eceff1")
    ...
    def _summarize_control_result(self, result: dict[str, object]) -> str:
        ...
        log_ref = debug_log_path(self.config.debug_log_root, str(summary.get("job_id", "gui-session")))
        parts = [
    ...
    def _set_program_row(self, program: str, *, status: str | None = None, detail: str | None = None) -> None:
    ...
    def _snapshot_warning(self, name: str, payload: dict[str, object] | None, fresh_sec: int) -> str | None:
        if payload is None:
    ...
    def _save_settings(self) -> None:
        ...
        _ = self.SETTINGS_FILE.write_text(json.dumps(payload, ensure_ascii=True, indent=2), encoding="utf-8")
    ...
    def toggle_pause(self) -> None:
        ...
        self._append_log("control loop paused" if self.paused else "control loop resumed")
    ...
    def trigger_recovery(self) -> None:
        ...
        self.last_result_text.set(f"recovery: {str(result.get('result', result))[:120]}")
    ...
    def seed_job(self, workload: GpuWorkload) -> None:
        ...
        if payload is None:
            return
        job = JobContract(job_id=f"{workload}-{uuid4()}", workload=workload, status="queued", payload=payload)
        ...
        while self.running and not self.stop_event.is_set():
            try:
                ...
                _ = append_debug_event(
                    debug_log_path(self.config.debug_log_root, str(result_summary.get("job_id", "gui-session"))),
                    event="gui_control_result",
                    level="ERROR" if str(result.get("status", "")) == "failed" else "INFO",
                    payload={
                        "owner": self.owner,
                        "result": result,
                    },
                ...
                if recovery is not None:
                    backoff_value = _to_float(recovery.get("backoff_sec", self.loop_sleep_sec), self.loop_sleep_sec)
    ...
    def refresh_dashboard(self) -> None:
        try:
            ...
            self.refresh_after_id = self.root.after(self.poll_ms, self.refresh_dashboard)

    def _run_control_once(self) -> dict[str, object]:
        if not self.control_lock.acquire(blocking=False):
            return {"status": "busy", "code": "CONTROL_BUSY", "result": "control loop busy"}
        try:
            ...
            if isinstance(sessions, list):
                ...
                for item in typed_sessions:
                    ...
                    if item_dict is None:
                        continue
                    row_name = row_map.get(str(item_dict.get("service", "")).strip().lower())
                    if row_name is None:
                        continue
                    ...
                    self._set_program_row(row_name, status=row_status, detail=f"port={port} fail={failures}")
    ...
    def _update_gpu_panel(self) -> None:
        ...
        self._set_program_row(row_name, status=str(payload.get("event", "대기")), detail=f"owner={owner}")
    ...
    def _update_queue_panel(self) -> None:
        ...
        for item in items[:100]:
            ...
            chain_depth = 0 if payload is None else _to_int(payload.get("chain_depth", 0))
            routed_from = "-" if payload is None else str(payload.get("routed_from", "-"))
            self.queue_list.insert(tk.END, f"{status:10} {workload:10} d={chain_depth} tries={attempts} from={routed_from} {job_id}")
    ...
    def _update_artifact_panel(self) -> None:
        ...
        if payload is None:
            ...
            self.artifact_text.set(f"artifacts: 0 / accepted={accepted} invalid={invalid}")
            return
        artifacts_obj = payload.get("artifacts", [])
        artifacts = cast(list[object], artifacts_obj) if isinstance(artifacts_obj, list) else []
        count = len(artifacts)
        ...
        completion_state = "-" if metadata is None else str(metadata.get("completion_state", "-"))
        final_output = False if metadata is None else bool(metadata.get("final_output", False))
        final_artifact = "" if metadata is None else str(metadata.get("final_artifact", ""))
        result_status = "-" if metadata is None else str(metadata.get("status", "-"))
        result_code = "-" if metadata is None else str(metadata.get("code", "-"))
        final_record = _latest_final_output_record(self.config.control_plane_events_file)
        last_final_artifact = "" if final_record is None else str(final_record.get("final_artifact", ""))
        inbox_root = self.config.input_root
        accepted = _archived_contract_count(inbox_root / "accepted")
        invalid = _archived_contract_count(inbox_root / "invalid")
    ...
    def _update_program_panel(self) -> None:
        ...
        if gui_payload is not None:
            ...
            if not worker_stage and status_payload is not None:
                worker_stage = str(status_payload.get("worker_stage", gui_payload.get("stage", "-"))).strip()
            ...
            backoff_sec = 0 if status_payload is None else _to_int(status_payload.get("backoff_sec", 0))
        ...
        if browser_health is not None:
            ...
            self._set_program_row(
                ...
                detail=f"availability={browser_health.get('availability_percent', 0)}% unhealthy={unhealthy}"[:40],
            )

    def _update_log_panel(self) -> None:
        event_records = _read_jsonl_tail(self.config.control_plane_events_file, limit=20)
        new_lines: list[str] = []
    ...
    def _update_warning_panel(self) -> None:
        ...
        browser_health = _read_json(self.config.browser_health_file)
        browser_warning = self._snapshot_warning("browser", browser_health, fresh_sec=120)
        ...
        browser_registry_warning = self._snapshot_warning("browser_registry", browser_registry, fresh_sec=120)
        if browser_registry_warning is not None:
            warnings.append(browser_registry_warning)
        if browser_health is not None and bool(browser_health.get("unhealthy_count", 0)):
        ...
        gpt_endpoints = [] if gpt_status is None or not isinstance(gpt_status.get("endpoints", []), list) else cast(list[object], gpt_status.get("endpoints", []))
        gpt_pending_boot = 0 if gpt_status is None else _to_int(gpt_status.get("pending_boot", 0))
        gpt_idle_bootstrap = len(gpt_endpoints) == 0 and gpt_pending_boot == 0
        if gpt_status is not None and bool(gpt_status.get("floor_breached", False)) and not gpt_idle_bootstrap:
        ...
        if gpu_health is not None and str(gpu_health.get("event", "")) in {"lock_busy", "renew_failed"}:
        ...
        if gui_payload is not None:
            invalid_reason = "" if status_payload is None else str(status_payload.get("invalid_reason", "")).strip()
            if invalid_reason:
