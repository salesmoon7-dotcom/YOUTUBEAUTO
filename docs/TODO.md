...
- latest service-coverage 기준으로는 `GeminiGen live auth/session validity`까지 proven이며 top-level deferred hold는 `Canva`입니다. fresh truly sequential rerun `D:\YOUTUBEAUTO_RUNTIME\probe\browser-recover-20260520-h\probe_result.json` + `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260520-h\probe_result.json`에서는 chain이 다시 `chatgpt -> qwen3_tts -> genspark ref-1 -> seaart ref-2 -> genspark main -> seaart main`까지 전진한 뒤 `Canva / CANVA_PRODUCT_BACKGROUND_NO_PROMPT_INPUT`에서 닫혔으므로, latest closeout truth는 service-coverage reading과 다시 정렬된 상태입니다.
...
- 2026-05-19 canva legacy top-dom retry result:
  ...
  - latest fresh boundary `D:\YOUTUBEAUTO_RUNTIME\probe\canva-boundary-20260520-b\probe_result.json`에서도 failed Canva row details는 여전히 `iframe_src`, `iframe_title`, `observed_frame_urls=[design edit, about:blank]`, `prompt_visible=false`, `file_select_visible=true`, `generate_visible=true`를 유지합니다. frame-committed OOPIF 경로까지 포함해 ref image 업로드 시도를 추가한 뒤에도 latest blocker는 그대로 `CANVA_PRODUCT_BACKGROUND_NO_PROMPT_INPUT`이며, worker failed details에는 `upload_attempted=true`, `upload_protocol_ok=true`가 함께 남아도 row details body는 업로드 성공 후 생성 UI로 전진하지 못한 채 OOPIF bootstrap/upload shell 상태에 머뭅니다.
