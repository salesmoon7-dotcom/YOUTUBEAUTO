...
- 최신 non-Canva 기준 남은 active gap은 `GeminiGen live login/session proof` 하나입니다. background-only probe는 이제 placeholder fallback을 success로 통과시키지 않고 `GEMINIGEN_LOGIN_UNPROVEN`으로 truthfully 닫히며, legacy `--repair-session` + browser recover 이후에도 `https://geminigen.ai/app/video-gen/grok` / `"Grok no longer free"? Access it here for FREE.` 상태에 머뭅니다.
