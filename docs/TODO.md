...
- latest service-coverage 기준으로는 `GeminiGen live auth/session validity`까지 proven이며 top-level deferred hold는 `Canva`입니다. 다만 fresh truly sequential rerun `D:\YOUTUBEAUTO_RUNTIME\probe\browser-recover-20260520-f\probe_result.json` + `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260520-e\probe_result.json`에서는 chain이 다시 `genspark / BROWSER_UNHEALTHY`에서 먼저 닫혔으므로, closeout latest truth는 여전히 `genspark browser/session boundary`와 `Canva OOPIF no-prompt upload state` 사이의 pinned boundary를 번갈아 드러내는 단계입니다.
...
- `docs/plans/2026-04-01-runtime-v2-closeout-retest-result.md` - latest semantic target row detached closeout fresh rerun은 `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260520-e\probe_result.json` + `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260520-e\evidence\failure_summary.json` 기준으로 다시 `chatgpt -> qwen3_tts` 이후 `genspark / BROWSER_UNHEALTHY`에서 contract상 닫혔습니다. 즉 row15 closeout latest truth는 아직 `genspark` 선행 불안정 경계와, `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260520-b\probe_result.json`에서 확인된 `Canva / CANVA_PRODUCT_BACKGROUND_NO_PROMPT_INPUT` 경계 사이를 오가며 pinned boundary를 좁히는 단계입니다. 이번 배치의 dynamic ref wiring은 `control_plane`이 `asset_manifest_path`를 `canva` job에도 전달하고 adapter child가 `image_primary`를 canva ref로 해석하도록 보강했지만, latest rerun은 여전히 `genspark`에서 먼저 닫혀 closeout 경로에서는 그 wiring이 아직 fully exercised되지 않았습니다.
