...
- 최신 non-Canva 기준 남은 active gap은 `GeminiGen live login/session proof` 하나입니다. background-only probe는 이제 placeholder fallback을 success로 통과시키지 않고 `GEMINIGEN_LOGIN_UNPROVEN`으로 truthfully 닫힙니다.
