...
- latest service-coverage 기준으로는 `GeminiGen live auth/session validity`까지 proven이며 top-level deferred hold는 `Canva`입니다. 다만 semantic row closeout latest chain은 fresh `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260519-e\probe_result.json` 기준으로 여전히 `genspark / BROWSER_UNHEALTHY`와 `Canva / PRODUCT_BACKGROUND_IFRAME_UNAVAILABLE` 사이의 pinned boundary를 번갈아 드러내고 있으므로, closeout truth는 line-level/chain-level evidence를 우선합니다.
...
- `docs/plans/2026-04-01-runtime-v2-closeout-retest-result.md` - latest semantic target row detached closeout은 fresh `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260519-e\probe_result.json` + `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260519-e\evidence\failure_summary.json` 기준으로 다시 `chatgpt -> qwen3_tts` 이후 `genspark / BROWSER_UNHEALTHY`에서 contract상 닫혔습니다. 다만 fresh `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260519-d\probe_result.json`에서는 pre-recover 직후 동일 체인이 `canva / PRODUCT_BACKGROUND_IFRAME_UNAVAILABLE`까지 전진한 evidence도 확보되어, 현재 closeout latest reading은 `genspark browser/session stability`와 `Canva iframe app-contract` 사이의 pinned boundary를 번갈아 확인하는 단계입니다.
