...
- latest service-coverage 기준으로는 `GeminiGen live auth/session validity`까지 proven이며 top-level deferred hold는 `Canva`입니다. fresh detached boundary `D:\YOUTUBEAUTO_RUNTIME\probe\canva-boundary-20260520-e\probe_result.json`에서는 same OOPIF shell이 `Canva / CANVA_PRODUCT_BACKGROUND_CREDIT_EXHAUSTED`로 더 truthfully 분류됩니다. 다만 latest truly sequential rerun `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260520-i\probe_result.json`은 다시 `genspark / BROWSER_UNHEALTHY`에서 먼저 닫혔으므로, closeout latest truth는 여전히 `genspark browser/session boundary`와 `Canva credit gate` 사이의 pinned boundary를 번갈아 드러내는 단계입니다.
...
- `docs/plans/2026-04-01-runtime-v2-closeout-retest-result.md` - latest semantic target row detached closeout fresh rerun은 `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260520-i\probe_result.json` + `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260520-i\evidence\failure_summary.json` 기준으로 다시 `chatgpt -> qwen3_tts` 이후 `genspark / BROWSER_UNHEALTHY`에서 contract상 닫혔습니다. 한편 그 직전 fully advanced chain `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260520-h\probe_result.json`과 paired polling `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260520-h-port-poll.jsonl`, 그리고 separate detached boundary `D:\YOUTUBEAUTO_RUNTIME\probe\canva-boundary-20260520-e\probe_result.json`는 `Canva / CANVA_PRODUCT_BACKGROUND_CREDIT_EXHAUSTED`까지 truth를 좁혀 둡니다. 즉 current closeout reading은 여전히 genspark 선행 불안정 경계와 Canva credit purchase gate 경계 사이를 오가며 pinned boundary를 좁히는 단계입니다.
