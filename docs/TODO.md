...
- pending external blocker: `Canva`는 현재 계정 크레딧이 소진된 상태라 fresh isolated boundary `D:\YOUTUBEAUTO_RUNTIME\probe\canva-boundary-20260524-a\runtime\artifacts\canva\canva-canva-boundary-20260524-a-3\canva-boundary-20260524-a\result.json`에서도 다시 `CANVA_PRODUCT_BACKGROUND_CREDIT_EXHAUSTED`로 닫힙니다. 사용자가 `현재 상태로 중단`을 선택했으므로, 이 축은 크레딧 충전 또는 크레딧이 있는 다른 계정 세션 제공 전까지 pending 외부 blocker로 유지합니다.
