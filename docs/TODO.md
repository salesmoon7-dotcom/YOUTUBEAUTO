...
- 최신 active migration 범위에서 non-Canva 쪽 실질 active gap은 닫혔습니다. fresh `D:\YOUTUBEAUTO_RUNTIME\probe\browser-recover-20260519-a\probe_result.json`은 `all_healthy=true` / `gpt_status.ok_count=3`을 보여 주고, fresh `D:\YOUTUBEAUTO_RUNTIME\probe\geminigen-login-proof-20260519-a\probe_result.json`은 `status=ok`, `code=OK`, `live_ready_services=["geminigen"]`, `results[2].status=ok`, `results[2].completion.final_output=true`로 닫힙니다. 따라서 current truthful reading은 `GeminiGen live auth/session validity`도 now proven이며, active migration 범위의 남은 top-level hold는 `Canva` 하나입니다.
...
- `docs/plans/2026-04-01-runtime-v2-closeout-retest-result.md` - 최신 semantic target row detached closeout 1회는 `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260519-c\probe_result.json` + `D:\YOUTUBEAUTO_RUNTIME\probe\stage5-row15-20260519-c\evidence\failure_summary.json`으로 다시 계약상 닫혔습니다. fresh browser recover `D:\YOUTUBEAUTO_RUNTIME\probe\browser-recover-20260519-e\probe_result.json`에서 `genspark`만 unhealthy로 시작해 재기동된 뒤에도, fresh row15 rerun은 다시 `chatgpt -> qwen3_tts`를 통과한 뒤 `genspark`에서 `BROWSER_UNHEALTHY`로 fail-close 되었습니다. 현재 active goal은 broad rerun이 아니라 이 `genspark` browser/session single boundary만 좁히는 것입니다.
