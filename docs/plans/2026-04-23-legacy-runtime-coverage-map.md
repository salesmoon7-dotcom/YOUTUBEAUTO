...
| Canva | `D:\YOUTUBE_AUTO\scripts\canva_automation.py` | `runtime_v2/stage2/canva_worker.py`, `runtime_v2/workers/agent_browser_worker.py`, `runtime_v2/cli.py` | Hold | even after restoring the legacy top-DOM `배경 생성 -> prompt -> 생성` preference, fresh `D:\YOUTUBEAUTO_RUNTIME\probe\canva-boundary-20260519-d\probe_result.json` still reports the Canva row truthfully as `failed / PRODUCT_BACKGROUND_IFRAME_UNAVAILABLE / fallback_used=true`; the remaining blocker is still a live Product Background iframe app-contract issue and must not be counted as done |
