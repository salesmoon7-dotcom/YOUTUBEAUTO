...
**Companion Plan:** 구조적 견고성/효율성 보강과 레거시 대비 보완 전략은 `docs/plans/2026-03-09-runtime-v2-architecture-robustness-review-plan.md`를 병행 기준으로 사용합니다. 이 문서는 서비스별 구현 순서를 고정하고, companion plan은 그 순서를 안전하게 밀어붙이기 위한 공통 아키텍처 선행조건을 고정합니다.
...
## Cross-Cutting Architecture Preconditions

아래 항목은 특정 서비스 하나의 기능 구현이 아니라, 모든 서비스 구현에 공통으로 적용되는 선행 구조 과제입니다. 서비스 순서와 별개가 아니라, 각 task 시작 전/후에 계속 확인해야 하는 고정 조건입니다.

1. **Single writer 유지**
   - `latest_active_run`, `latest_completed_run`, `result.json`, `gui_status`, `events`의 최종 의미는 단일 owner 아래에서만 갱신되어야 합니다.
   - 새 service task를 진행하기 전에, 해당 변경이 writer 경로를 하나 더 만들지 않는지 먼저 확인합니다.

2. **Failure contract 의미 고정**
   - `status`, `error_code`, `retryable`, `completion.state`는 각각 하나의 의미만 가져야 합니다.
   - worker는 policy를 결정하지 않고 contract/evidence만 반환하며, blocked/hold/backoff 정책은 runtime/control-plane에서만 결정합니다.

3. **Adapter boundary 단일화**
   - service별 adapter 호출은 장기적으로 단일 helper/단일 reference adapter 경로로 수렴시켜야 합니다.
   - 임시 `adapter_command` 성공 seam을 추가할 때도, 그것이 다음 service들에서 별도 패턴으로 갈라지지 않도록 먼저 점검합니다.

4. **Mock/debug 경로 격리**
   - `mock_chain`, probe, seed-only 경로는 운영 기본 경로와 의미가 섞이지 않도록 유지합니다.
   - 테스트 편의를 위한 mock 경로가 운영 성공 근거로 재사용되지 않게 문서/테스트에서 계속 분리합니다.

5. **Legacy compensation은 임시 장치로만 사용**
   - 레거시의 장점(coverage, 실제 산출 책임)은 비교 기준으로 흡수하되, 정책/상태/재시도/보정 로직의 분산 구조는 재도입하지 않습니다.
   - parity 비교나 임시 보정은 허용하되, 제거 조건과 종료 기준이 같이 문서화되어야 합니다.
   - 허용되는 보정은 `reference-only comparison`, `evidence diff`, `targeted regression`처럼 단일 owner를 늘리지 않는 형태로 한정합니다.
   - legacy helper/module direct import, legacy retry branch porting, legacy state model 재사용은 금지합니다.

위 5개 중 하나라도 흔들리면, 현재 service task의 기능 확장보다 companion robustness plan의 관련 task를 먼저 수행합니다.
...
- `24h 실제 운영 완료 증거`는 현 구현 세션의 즉시 완료 항목이 아니라 훨씬 뒤의 운영/soak 단계입니다. 따라서 하부프로그램 core logic 구현 중에는 이 증거를 반복 확인하거나 현재 완료 조건으로 재질문하지 않습니다.
...
- 이 단계는 현재 구현 스프린트의 근접 목표가 아닙니다.
- 실제 장시간 운영, soak scheduling, 운영 리포트 수집이 필요하므로 앞선 서비스 core logic 구현이 충분히 진행되기 전에는 착수/질문/완료 판정 대상으로 삼지 않습니다.
- 세션 중간 점검에서는 `24h 증거가 아직 없음`을 반복 보고하지 않고, 단순히 `후순위 운영 단계`로 유지합니다.
