...
**Post-Close Follow-Up Audit:** 같은 세션의 추가 조사에서 residual drift 6건을 더 확인했고 즉시 닫았습니다. (1) `runtime_v2/stage1/chatgpt_runner.py`가 `route_video_plan()`으로 downstream `next_jobs`를 직접 생성하던 policy leakage, (2) `runtime_v2/latest_run.py` / `runtime_v2/cli.py`가 CLI 경로에서 latest pointer를 직접 갱신하던 ownership drift, (3) `runtime_v2/cli.py`의 stage2 adapter child가 검증 실패 후 내부 recovery tick + 재검증을 수행하던 policy drift, (4) `runtime_v2/control_plane.py`의 queue persistence가 `QueueStore`와 private `_load_jobs/_save_jobs/_upsert_job` 사이에 중복되어 있던 ownership drift, (5) `runtime_v2/latest_run.py` / `runtime_v2/manager.py`의 excel sync snapshot이 latest pointer를 직접 갱신하던 drift, (6) `runtime_v2/queue_store.py`가 손상된 queue JSON을 조용히 빈 큐로 취급할 수 있던 robustness gap입니다. 이 항목들은 control-plane / canonical helper / explicit fail-closed contract에 다시 수렴시켰고, corrupted queue는 `QUEUE_STORE_INVALID`로 surface 되도록 정리했습니다.
