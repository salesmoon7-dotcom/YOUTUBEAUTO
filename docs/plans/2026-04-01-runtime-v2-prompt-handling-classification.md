...
|---|---|---|
| `chatgpt` | `pass-through` | `runtime_v2/stage1/chatgpt_runner.py` `build_live_chatgpt_prompt()` sends only the stripped row topic. Stage1 structure is required after capture by `runtime_v2/stage1/gpt_response_parser.py`, not by hidden prompt strengthening. |
...
- `chatgpt` stays limited to legacy/topic-only input: the row topic is the only semantic input, and the runtime requires captured output to satisfy either the Stage1 JSON contract or supported legacy free-form blocks before downstream handoff.
...
- If `chatgpt` prompt semantics change again, both this table and the Stage 1 prompt contract must be updated together. Parser-side fail-closed behavior must remain the owner for missing `scene_prompts` / `voice_groups`.
