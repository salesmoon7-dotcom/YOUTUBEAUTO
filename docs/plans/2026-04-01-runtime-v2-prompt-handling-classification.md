...
|---|---|---|
| `chatgpt` | `pass-through` | `runtime_v2/stage1/chatgpt_runner.py` `build_live_chatgpt_prompt()` forwards the topic text itself on the current runtime_v2 path. This is distinct from legacy `chatgpt_automation.py`, which assembles a separate `request_text` wrapper before sending. |
...
- `chatgpt` now stays `pass-through` at the runtime_v2 browser input boundary; legacy automation may still assemble a wrapper upstream, so these two paths should not be conflated.
