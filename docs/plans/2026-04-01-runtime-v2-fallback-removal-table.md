...
| `genspark` fresh-result-tab preference logic in `runtime_v2/agent_browser/cdp_capture.py` | `REMOVED` | narrowed capture to the newest plausible `agents?id=` result tab while caller/service contract remained broad | caller-side capture now pins a single verified `agents?id=` result URL and fails closed with `GENSPARK_RESULT_TAB_UNPINNED` when that URL cannot be proven | removed after capture layer was reduced to generic exact-URL matching and no compose/stale-tab fallback remained reachable |
...
- The current fallback-removal pass no longer retains service-specific genspark capture selection logic.
